...
`forex_agent.infrastructure.broker_oanda._shared.require_practice_host`.
Provider-specific objects (raw JSON, httpx types) never escape this module:
every method returns a `domain` value object or raises one of
`forex_agent.application.ports.exceptions`.
"""
