"""FX-20: VolatilityExpansionBreakoutStrategy run through run_backtest +
simulate_trades against real OANDA practice candles. Auto-skipped when
OANDA credentials aren't configured — same pattern as FX-14/15/16/19's
live tests.
"""
...
@pytest.mark.asyncio
async def test_volatility_expansion_against_live_practice_candles(
    ...
    strategy = VolatilityExpansionBreakoutStrategy(
        short_period=14, long_period=50, breakout_lookback=20, expansion_threshold=Decimal("1.5")
    )
