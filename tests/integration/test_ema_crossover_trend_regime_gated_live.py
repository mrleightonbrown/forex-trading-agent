"""FX-28: EmaCrossoverTrendRegimeGatedStrategy run through run_backtest +
simulate_trades against real OANDA practice candles — proves the whole
pipeline works with a real strategy, not just synthetic fixtures. Same
pattern as FX-14's own live test. Auto-skipped when OANDA credentials
aren't configured.
"""
...
@pytest.mark.asyncio
async def test_ema_crossover_trend_regime_gated_against_live_practice_candles(
    ...
    strategy = EmaCrossoverTrendRegimeGatedStrategy(fast_period=20, slow_period=50)
    hypotheses = run_backtest(strategy, finalized)
    trades = simulate_trades(hypotheses, finalized)

    # Not asserting *how many* signals fired or how many were gated to
    # FLAT vs confirmed -- that depends on live market behavior -- only
    # that whatever came out is internally consistent with what the
    # strategy and simulator guarantee.
    for hypothesis in hypotheses:
        assert hypothesis.strategy_key == "ema_crossover_trend_regime_gated_v1"
        assert hypothesis.instrument == eur_usd
        assert hypothesis.target_position in (
            TargetPosition.LONG,
